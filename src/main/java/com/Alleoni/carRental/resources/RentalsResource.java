...
public class RentalsResource {
	...
	public ResponseEntity<List<Rentals>> findAll() {
		List<Rentals> list = service.findAll();
		return ResponseEntity.ok().body(list);
	}

	@GetMapping(value = "/{id}")
	public ResponseEntity<Rentals> findById(@PathVariable Long id){
		Rentals obj = service.findById(id);
		return ResponseEntity.ok().body(obj);
	}
	
	@PostMapping
	public ResponseEntity<Rentals> insert(@RequestBody Rentals rental){
	...
	@PutMapping(value = "{id}")
	public ResponseEntity<Rentals> update(@PathVariable Long id, @RequestBody Rentals rental){
